...
echo "[1/1] Training depth_promptfusion_only"
torchrun --standalone --nproc_per_node=4 \
  -m ft.kitti_stage1_5f.tools.train \
  --exp depth_promptfusion_only \
  --processed_root "${PROCESSED_ROOT}" \
  --occany_ckpt "${OCCANY_CKPT}" \
  --output_dir /home/dataset-local/lr/code/OccAny/output/kitti_stage1_5f_4gpu_depth_promptfusion_only \
  --no-freeze_backbone
